...
\drop{E}{n} este capítulo se describe el proceso de desarrollo del sistema de navegación por
...
posee la mayor cuota de mercado (81\%) y dispone de una gran variedad de dispositivos en una
...
seleccionó Open Street Map. Se consideró la mejor elección porque provee de mapas de gran
...
civil hasta 2012 no todos los \emph{smartphones} disponen en la actualidad de esta tecnología y deja
una única opción razonable: \acs{NAVSTAR-GPS}.
...
Para implementarlo en el sistema, se seleccionó la segunda opción porque se consideró que era
...
\begin{itemize}
  ...
  \item Cómo \textbf{detectar si se sigue la ruta marcada}. Hay que tener en cuenta que sólo se
    ...
    llegue a la que se encuentra realizando, es posible que no funcione. Por ejemplo, si un usuario
    se acerca a una etapa de \emph{girar a la derecha} y gira en el sitio correcto antes de que la
    aplicación detecte que ha llegado (probablemente por la precisión de los satélites \acs{GPS}),
    no cargaría la siguiente etapa y no sabría qué está sucediendo.
...
public void onLocationChanged(Location loc) {  
  ...
  \item Para \textbf{detectar si se sigue la ruta marcada} se ha implementado el algoritmo del
...
\begin{itemize}
  ...
  \item Para comprobar el correcto funcionamiento del sistema cuando el usuario se pierde
